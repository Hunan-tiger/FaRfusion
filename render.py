...
def kalman_smooth_params(data, ):# data:[seqlen, d]
...
class Render(object):
    ...
    def rendering_for_fid(self, path, ind, listener_vectors, speaker_video_clip, listener_reference, listener_video_clip):
        # 3D video
        T = listener_vectors.shape[0]
        #listener_vectors = self._reverse_transform_3dmm(listener_vectors)[0]    dataset.py没有减去mean_face

        self.faceverse.batch_size = T
        self.faceverse.init_coeff_tensors()

        listener_vectors_copy = listener_vectors.clone()
        listener_vectors[:, :] = torch.from_numpy(kalman_smooth_params(listener_vectors[:, :].cpu().numpy())).to(listener_vectors_copy.get_device()).to(torch.float32)
        #self.faceverse.exp_tensor = listener_vectors[:, :52].view(T, -1).to(listener_vectors.get_device())
        self.faceverse.exp_tensor = listener_vectors[:, :52].view(T, -1)
        self.faceverse.rot_tensor = listener_vectors[:, 52:55].view(T, -1)
        self.faceverse.trans_tensor = listener_vectors[:, 55:].view(T, -1)
        ...
        out = cv2.VideoWriter(os.path.join(path, 'results_videos', ind + "_val.avi"), cv2.VideoWriter_fourcc(*"MJPG"), 25, (896, 224))
        for i in range(rendered_img_r.shape[0]):
            combined_img = np.zeros((224, 896, 3), dtype=np.uint8)
            combined_img[0:224, 0:224] = speaker_video_clip[i]
            combined_img[0:224, 224:448] = rendered_img_r[i]
            combined_img[0:224, 448:672] = listener_videos[i]
            combined_img[0:224, 672:] = listener_video_clip[i]
            out.write(combined_img)
        out.release()
