...
class MLPLNAct(nn.Module):
    def __init__(
        ...
        if self.use_cond:
            self.linear_emb = nn.Linear(cond_channels, out_channels*3)
    ...
    def forward(self, x, cond=None):
        x = self.linear(x)
        if self.use_cond:
            shift, scale, gate = self.cond_layers(cond).chunk(3, dim=1)
            # scale shift first
            x = x + gate.unsqueeze(1) * modulate(x, shift, scale)
            self.norm(x)
...
class MLPSkipNet(nn.Module):
    ...
    """
    def __init__(self, 
                num_channels=128, # this is the number of features in the input (denoising target)
                skip_layers='all',
                num_hid_channels=1024,
                num_layers=20,
                num_cond_emb_channels=128, # this is the number of features in the cond embedding
                num_time_emb_channels=128, # this is the number of features in the time embedding
    ...
    def forward(self, x, t, cond):
        t = timestep_embedding(t, self.num_time_emb_channels, dtype=self.dtype)  # 根据t生成正弦位置编码

        time_emb = self.time_embed(t)
        cond_emb = self.cond_embed(cond)
        # print("AFTER", "timestep embedding", time_emb.shape, "cond", cond_emb.shape)
        cond = time_emb + cond_emb
